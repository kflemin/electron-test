#start the measure
class SetExteriorRoofAssemblytoUserInputUValue < OpenStudio::Ruleset::ModelUserScript

  #define the name that a user will see
  def name
    return "Set Exterior Roof Assembly to User Input U Value"
  end
  ...
  def arguments(model)
    args = OpenStudio::Ruleset::OSArgumentVector.new
		...
		#putting construction into hash
		surface_args = model.getSurfaces
		construction_args_hash = {}
		surface_args.each do |surface_arg|
		construction_arg = surface_arg.construction.get
		...
		construction_args_hash.sort.map do |key,value| #todo - could filter this so only constructions that are valid on opaque surfaces will show up.
		  unless key.include? "Window" or key.include? "Interior" or key.include? "Wall" or key.include? "Door" or key.include? "Slab"
    ...
    construction.setDisplayName("Pick Exterior Opaque Construction to Change")
    args << construction
	
		#make an argument assembly U value
    u_value = OpenStudio::Ruleset::OSArgument::makeDoubleArgument("u_value",true)
    u_value.setDisplayName("New Assembly U-value (Btu/h*ft^2*R).")
    u_value.setDefaultValue(0.064)
    args << u_value
  ...
  def run(model, runner, user_arguments)
    ...
    u_value = runner.getDoubleArgumentValue("u_value",user_arguments)
		...
		construction_handle = runner.getStringArgumentValue("construction",user_arguments)
    ...
    if r_value_constr < 0 or r_value_constr > 500
      runner.registerError("The requested construction assembly U-value of #{u_value} Btu/h*ft^2*R was out of measure limit.")
      return false
    elsif r_value_constr > 50
      runner.registerWarning("The requested construction assembly U-value of of #{u_value} Btu/h*ft^2*R is abnormally low.")
    elsif r_value_constr < min_expected_r_value_ip
      runner.registerWarning("The requested construction assembly U-value of of #{u_value} Btu/h*ft^2*R is abnormally high.")
    end
    ...
    r_value_constr_si = unit_helper(r_value_constr, "ft^2*h*R/Btu","m^2*K/W")
    ...
    ext_wall_resistance = []
      ...
      if construction.get.to_Construction.is_initialized
      ...
      if surface.outsideBoundaryCondition == "Outdoors" and surface.surfaceType == "Wall" 
        ...
        #runner.registerInfo("#{surface.name} has construction type #{surface.construction.get.name.to_s}")
			if not surface.construction.empty?
				 if ext_wall_const.name.to_s == construction.name.to_s
					...
					ext_wall_resistance << 1/ext_wall_const.thermalConductance.to_f
							...
							registerWarning("#{surface.name} does not have a construction")	
					end
			end
    end
    ...
    exterior_surface_constructions.each do |exterior_surface_construction|
      ...
      if not thermal_resistance_values.max > unit_helper(min_expected_r_value_ip, "ft^2*h*R/Btu","m^2*K/W")
        runner.registerWarning("Construction '#{exterior_surface_construction.name.to_s}' does not appear to have an insulation layer and was not altered.")
				 # elsif not thermal_resistance_values.max < r_value_ins_si and not allow_reduction
				...
				# calculated other layers' combined R value
				r_value_other_layers = ext_wall_resistance[1] - thermal_resistance_values.max	
				#thermal_resistance_values_max_ip = unit_helper(thermal_resistance_values.max, "m^2*K/W","ft^2*h*R/Btu")
				#ext_wall_resistance_ip = unit_helper(ext_wall_resistance[1], "m^2*K/W","ft^2*h*R/Btu")
				#runner.registerInfo("existing wall assembly R value is #{ext_wall_resistance_ip} with insulation layer R value #{thermal_resistance_values_max_ip}")
				r_value_ins_new = r_value_constr_si - r_value_other_layers
				r_value_ins_new_ip = unit_helper(r_value_ins_new, "m^2*K/W","ft^2*h*R/Btu")
        ...
        if found_material == false
          ...
          new_material.setName("#{max_thermal_resistance_material.name.to_s}_R-value to: #{r_value_ins_new_ip} (ft^2*h*R/Btu)")
          materials_hash[max_thermal_resistance_material.name.to_s] = new_material
          final_construction.eraseLayer(max_thermal_resistance_material_index)
          final_construction.insertLayer(max_thermal_resistance_material_index,new_material)
          ...
          if not new_material_matt.empty?
            ...
            target_thickness = starting_thickness * r_value_ins_new / thermal_resistance_values.max
            final_thickness = new_material_matt.get.setThickness(target_thickness)
          end
          new_material_massless = new_material.to_MasslessOpaqueMaterial
          if not new_material_massless.empty?
            final_thermal_resistance = new_material_massless.get.setThermalResistance(r_value_ins_new)
          end
          new_material_airgap = new_material.to_AirGap
          if not new_material_airgap.empty?
            final_thermal_resistance = new_material_airgap.get.setThermalResistance(r_value_ins_new)
          end
    ...
    runner.registerFinalCondition("The existing insulation for exterior walls was set to R-#{1/u_value}. This was accomplished for an initial cost of #{one_time_retrofit_cost_ip} ($/sf) and an increase of #{material_cost_increase_ip} ($/sf) for construction. This was applied to #{neat_numbers(affected_area_ip,0)} (ft^2) across #{final_string.size} exterior wall constructions: #{final_string.sort.join(", ")}.")
...
SetExteriorRoofAssemblytoUserInputUValue.new.registerWithApplication
